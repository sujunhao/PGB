...
function getReadyStateHandler() {
    if (req.readyState == 4) {
        if (req.status == 200) {
            ...
            theSecondId = data[5].getAttribute('id');
            theStep = parseInt(data[5].childNodes[2].innerHTML);
            var kk = (data[5].childNodes[3].innerHTML).split(';');
            for (var i=0; i<kk.length; i++){theV[i] = parseFloat(kk[i]);}
